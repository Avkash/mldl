# All Code Snippets #

## Python ##
 - [Cars and MPG Machine Learning Demo](https://github.com/Avkash/mldl/blob/master/code/python/cars_mpg_demo.md)
 - [Common python function specific to Java](https://github.com/Avkash/mldl/blob/master/code/python/java_helper.md)
 - [Pysparkling Full end to end test Script](https://github.com/Avkash/mldl/blob/master/code/python/pysparkling_test.md)
 - [Python sample code to perform MOJO Scoring](https://github.com/Avkash/mldl/blob/master/code/python/python_mojo_score.md)
 - [Sample Python Machine Learning Code](https://github.com/Avkash/mldl/blob/master/code/python/sample_python_ml.md)
 - [Saving H2O Model object into text file](https://github.com/Avkash/mldl/blob/master/notebook/h2o/H2O_Model_Output_Save_Local_Disk.ipynb)
 - [How R2 error is calculated in Generalized Linear Model](https://github.com/Avkash/mldl/blob/master/notebook/h2o/CALCULATING_GLM_MODEL_METRICS.ipynb)

## Pysparkling ##
 - [H2O Pysparkling Exectuion with different port](https://github.com/Avkash/mldl/blob/master/orgs/h2o/pysparkling_new_port.md)
 
## Scala ##
 - [H2O AutoML Sample](https://github.com/Avkash/mldl/blob/master/code/scala/automl_scala_sample.md)
 - [H2O Word2Vec Sample](https://github.com/Avkash/mldl/blob/master/code/scala/h2o_word2vec_ex.md)
 - [GBM Regression and Classification Scala Sample](https://github.com/Avkash/mldl/blob/master/code/scala/prostate_gbm_sample.md)
 - [GBM example to perform GridSearch with Hyper parameters](https://github.com/Avkash/mldl/blob/master/code/scala/scala_gbm_gridsearch_hyper_h2o.md)
 - [MOJO Scoring with using Scala and Spark](https://github.com/Avkash/mldl/blob/master/code/scala/spark_udf_score.md)
 - [Handling nested parquet with columns as Array](https://github.com/Avkash/mldl/blob/master/code/scala/nested-parquet-handler.md)
 
## Java ## 
 - [Java code to use MOJO with Binomial model](https://github.com/Avkash/mldl/blob/master/code/java/mojo-usage.md)
 - [Java code to use POJO with Binomial model](https://github.com/Avkash/mldl/blob/master/code/java/pojo-usage.md)
 - [Java code to use POJO with GBM Binomial model](https://github.com/Avkash/mldl/blob/master/code/java/pojo_gbm_binomial.md)
 - [Java code to use POJO with GLM Binomial model](https://github.com/Avkash/mldl/blob/master/code/java/pojo_glm_regression.md)

## R ##
 - [rSparkling Install Guide](https://github.com/Avkash/mldl/blob/master/code/R/rsparkling_guide.md)
 - []()

## Linux  and Libraries - Tips and Tricks ##
 - [Netezza Tips and Tricks](https://github.com/Avkash/mldl/blob/master/code/netezza_tips.md)
 - [postgresql Tips and Tricks](https://github.com/Avkash/mldl/blob/master/code/psql_utils.md)
 - [Herringbone: flatten nested parquet files on Hadoop](https://github.com/Avkash/mldl/blob/master/code/linux/harringbone_work.md)
 - [OSX Command Line Tricks](https://github.com/Avkash/mldl/blob/master/code/linux/OSX_cli.md)
 
## Hadoop Tips and Tricks ##
 - [Hadoop and HDFS Tips and Tricks](https://github.com/Avkash/mldl/blob/master/code/linux/hadoop_tricks.md)
 - [H2O cluster launch issue due to missing h2odriver.jar file](https://github.com/Avkash/mldl/blob/master/code/linux/hadoop_error_1.md)

